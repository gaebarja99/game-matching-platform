...
분리형 아키텍처: Spring Boot API 서버와 분리된 React SPA입니다. 기본 API는 `http://localhost:8080`(프론트는 보통 `http://localhost:5173`).
...
- `VITE_API_URL`: API 서버 주소 (`.env.example` 기본 `http://localhost:8080`)
- `.env` 파일에 설정하거나, 빌드 시 지정 가능.

## 로그인

- **일반 로그인**: `/login`에서 아이디/비밀번호 제출 → API `POST /api/auth/login` (세션 쿠키)
- **구글 로그인**: "Google로 로그인" 클릭 → API 서버 `GET /oauth2/authorization/google`로 이동 → 로그인 완료 후 백엔드가 `app.frontend.url`(기본 localhost:5173 등)로 리다이렉트
