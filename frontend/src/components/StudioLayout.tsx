...
export default function StudioLayout({ children }: StudioLayoutProps) {
  ...
  // 현재 경로에 해당하는 그룹을 항상 열어 두기 (고정)
  useEffect(() => {
    const groupForKeyPath: Record<string, boolean> = {};
    if (path.startsWith('/studio/live') || path.startsWith('/studio/settings') || path.startsWith('/studio/alerts') || path.startsWith('/studio/chat')) {
      groupForKeyPath.broadcast = true;
    }
    if (path.startsWith('/studio/analysis')) groupForKeyPath.analysis = true;
    if (path.startsWith('/studio/viewers')) groupForKeyPath.viewers = true;
    if (path.startsWith('/studio/revenue') || path.startsWith('/studio/admin')) groupForKeyPath.revenue = true;
    if (Object.keys(groupForKeyPath).length > 0) {
      setOpenGroups((prev) => {
        const next = { ...prev, ...groupForKeyPath };
        saveOpenGroups(next);
        return next;
      });
    }
              ...
              <div className="studio-header-profile-dropdown show">
                <div className="studio-dropdown-profile-info">
                  ...
                  <div>
                    ...
                    <div className="studio-dropdown-name">{user?.nickname ?? user?.username ?? user?.loginId ?? '—'}</div>
                  </div>
                </div>
                <div className="studio-dropdown-menu">
                  <Link to="/profile" onClick={() => setProfileOpen(false)}>
                    <svg className="menu-icon" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    내 프로필
                  </Link>
                  <Link to="/studio" onClick={() => setProfileOpen(false)}>
                    <svg className="menu-icon" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><rect x="2" y="4" width="14" height="14" rx="2"/><path d="M17 8v8l5-4z"/></svg>
                    스튜디오
                  </Link>
                  <Link to="/channel" onClick={() => setProfileOpen(false)}>
                    <svg className="menu-icon" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>
                    내 채널
                  </Link>
                  <Link to="/following" onClick={() => setProfileOpen(false)}>
                    <svg className="menu-icon" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 18.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                    팔로잉 채널
                  </Link>
                  <button type="button" onClick={handleLogout}>
                    <svg className="menu-icon" viewBox="0 0 24 24" stroke="currentColor" fill="none" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
                    로그아웃
                  </button>
                </div>
              </div>
        ...
        <aside className="studio-sidebar" style={{ display: sidebarVisible ? undefined : 'none' }}>
          ...
          <div className="nav-section">
            ...
            <div className={`nav-group ${openGroups.analysis ? 'open' : ''}`}>
              ...
              <ul className="nav-sub">
                <li>
                  <Link to="/studio/analysis/live" className={path === '/studio/analysis/live' ? 'active' : ''}>라이브 분석</Link>
                </li>
                <li>
                  <Link to="/studio/analysis/video" className={path === '/studio/analysis/video' ? 'active' : ''}>동영상 분석</Link>
                </li>
              </ul>
            </div>
          </div>
          <div className="nav-section">
            ...
            <div className={`nav-group ${openGroups.viewers ? 'open' : ''}`}>
              <button
                type="button"
                className="nav-item nav-trigger"
                onClick={() => toggleGroup('viewers')}
              >
                <span className="icon">👥</span> 시청자 관리 <span className="nav-arrow">▼</span>
              </button>
              <ul className="nav-sub">
                <li>
                  <Link to="/studio/viewers/followers" className={path === '/studio/viewers/followers' ? 'active' : ''}>팔로워</Link>
                </li>
                <li>
                  <Link to="/studio/viewers/subscribers" className={path === '/studio/viewers/subscribers' ? 'active' : ''}>구독자</Link>
                </li>
                <li>
                  <Link to="/studio/viewers/blocklist" className={path === '/studio/viewers/blocklist' ? 'active' : ''}>활동 제한</Link>
                </li>
              </ul>
            </div>
          </div>
          <div className="nav-section">
            ...
            <div className={`nav-group ${openGroups.content ? 'open' : ''}`}>
              ...
              <ul className="nav-sub">
                ...
                <li>
                  <a href="#" onClick={(e) => { e.preventDefault(); alert('준비 중입니다.'); }}>동영상</a>
                </li>
                <li>
                  <a href="#" onClick={(e) => { e.preventDefault(); alert('준비 중입니다.'); }}>내가 만든 클립</a>
                </li>
                <li>
                  <a href="#" onClick={(e) => { e.preventDefault(); alert('준비 중입니다.'); }}>내 채널의 클립</a>
                </li>
              </ul>
            </div>
          </div>
          <div className="nav-section">
            ...
            <div className={`nav-group ${openGroups.revenue ? 'open' : ''}`}>
              ...
              <ul className="nav-sub">
                <li>
                  <Link to="/studio/revenue" className={path === '/studio/revenue' ? 'active' : ''}>수익 창출</Link>
                </li>
                {user?.role === 'ADMIN' && (
                  <li>
                    <Link to="/studio/admin/streamers" className={path === '/studio/admin/streamers' ? 'active' : ''}>스트리머 관리</Link>
                  </li>
                )}
              </ul>
            </div>
          ...
          <div className="nav-section">
            ...
            <div className={`nav-group ${openGroups.channel ? 'open' : ''}`}>
              ...
              <ul className="nav-sub">
                <li>
                  <a href="#" className="ext" onClick={(e) => { e.preventDefault(); alert('준비 중입니다.'); }}>채널 관리</a>
                </li>
                <li>
                  <a href="#" onClick={(e) => { e.preventDefault(); alert('준비 중입니다.'); }}>권한 관리</a>
                </li>
              </ul>
            </div>
          </div>
