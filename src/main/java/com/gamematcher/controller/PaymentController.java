...
public class PaymentController {
    ...
    @PostMapping("/adfree/orders")
    public ResponseEntity<?> createAdFreeOrder(HttpSession session) {
        Long userId = (Long) session.getAttribute(SESSION_USER_ID);
        if (userId == null) {
            return ResponseEntity.status(401).build();
        }
        try {
            PaymentService.CreateOrderResult result = paymentService.createAdFreeOrder(userId);
            return ResponseEntity.ok(Map.of(
                    "orderId", result.orderId(),
                    "amount", result.amountWon(),
                    "orderName", result.orderName(),
    ...
    @PostMapping("/adfree/confirm")
    public ResponseEntity<?> confirmAdFree(@RequestBody ConfirmPaymentRequest request, HttpSession session) {
        Long userId = (Long) session.getAttribute(SESSION_USER_ID);
        if (userId == null) {
            return ResponseEntity.status(401).build();
        }
        ...
        try {
            PaymentService.ConfirmResult result = paymentService.confirmAdFreePayment(userId, request.getOrderId(), impUid);
            return ResponseEntity.ok(Map.of(
                    "success", result.success(),
                    "balance", result.balance(),
                    "message", result.message()
