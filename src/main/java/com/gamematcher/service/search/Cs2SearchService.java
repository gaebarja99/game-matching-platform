            ...
            try {
                ...
                if (code == 400) {
                    return PlayerSearchResponse.error("cs2", displayName,
                            "CS2 통계를 불러올 수 없습니다.\n"
                                    + "· Steam 프로필 → 게임 세부 정보를 '공개'로 설정했는지 확인하세요.\n"
                                    + "· CS2(App 730)를 한 번 이상 플레이했는지 확인하세요.");
                } else if (code == 403) {
                    return PlayerSearchResponse.error("cs2", displayName,
                            "CS2 통계가 비공개입니다. Steam 프로필 → 게임 세부 정보를 공개로 설정하세요.");
                }
                throw ex;
            }
