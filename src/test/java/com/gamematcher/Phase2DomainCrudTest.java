...
import com.gamematcher.constant.*;
import com.gamematcher.dto.stats.*;
...
class Phase2DomainCrudTest {
    ...
    @Autowired com.gamematcher.repository.UserRepository userRepository;
    @Autowired StatsConverter statsConverter;

    @Test
    @DisplayName("Game CRUD - 게임 생성 및 조회")
    void gameCrud() {
        Game valorant = new Game("발로란트", GameList.VALORANT, "{}");
        Game saved = gameRepository.save(valorant);

        assertThat(saved.getId()).isNotNull();
        assertThat(saved.getCode()).isEqualTo(GameList.VALORANT);
        assertThat(gameRepository.findByCode(GameList.VALORANT)).contains(saved);
    }

    @Test
    @DisplayName("MatchRecord CRUD - 매치 기록 생성 및 조회")
    void matchRecordCrud() {
        Game game = gameRepository.save(new Game("롤", GameList.LEAGUE_OF_LEGENDS, null));
        MatchRecord record = new MatchRecord(
            game, "match-001", "VICTORY",
            LocalDateTime.now(),
    ...
    void participantWithRawStatsAndConverter() {
        ...
        Game game = gameRepository.save(new Game("발로란트", GameList.VALORANT, null));
        MatchRecord record = matchRecordRepository.save(
            new MatchRecord(game, "match-v1", "VICTORY", LocalDateTime.now(), null)
        );

        ValorantStatsDTO stats = new ValorantStatsDTO(
            GameList.VALORANT, 20, 5, 10, true,
            13, 24, 8, 40
        );
    ...
    void lolStatsConverterRoundTrip() {
        LolStatsDTO original = new LolStatsDTO(
            GameList.LEAGUE_OF_LEGENDS, 8, 2, 15, true,
            12500, 180, 25000L, 45, 32
        );
        String json = statsConverter.toJson(original);
        BaseStatsDTO parsed = statsConverter.toDto(json, GameList.LEAGUE_OF_LEGENDS);
    ...
    void evaluationCrud() {
        ...
        Game game = gameRepository.save(new Game("발로란트", GameList.VALORANT, null));
        MatchRecord record = matchRecordRepository.save(
            new MatchRecord(game, "match-eval", "VICTORY", LocalDateTime.now(), null)
        );
        ...
        MatchRecordEvaluation eval = new MatchRecordEvaluation(
            ...
            120, "S", "훌륭한 킬 수", "상세 코멘트..."
        );
        MatchRecordEvaluation saved = evaluationRepository.save(eval);

        assertThat(saved.getScore()).isEqualTo(120);
        assertThat(saved.getGrade()).isEqualTo("S");
    ...
    void userEvaluationsPagination() {
        ...
        Game game = gameRepository.save(new Game("롤", GameList.LEAGUE_OF_LEGENDS, null));
        MatchRecord record = matchRecordRepository.save(
            new MatchRecord(game, "match-paged", "VICTORY", LocalDateTime.now(), null)
        );
        MatchRecordParticipant participant = participantRepository.save(
            new MatchRecordParticipant(record, user, "Mid", "{}")
        );
        evaluationRepository.save(new MatchRecordEvaluation(
            participant, EvaluationStatus.COMPLETED, 95, "A", "요약", "상세"
        ));
