...
챗봇 답변 톤을 더 짧고 깔끔하게 정리했습니다.
답변에 📌, 👉, ✅ 같은 이모지 섹션을 적용해서 가독성을 높였습니다.
웹사이트 구현 범위 밖 질문이나 불필요한 잡담에는 도와드릴 내용이 있으면 말씀해 주세요.로 응답하도록 정리했습니다.
다만 너 뭔데?, 누구세요?, 무슨 챗봇이야? 같은 정체 질문은 예외로 처리해서 챗봇이 자신이 어떤 역할인지 설명하도록 수정했습니다.
챗봇 답변의 줄바꿈과 목록이 화면에서 더 보기 좋게 표시되도록 렌더링 방식을 개선했습니다.
챗봇 전용 페이지와 우측 하단 플로팅 챗봇 위젯에 동일한 답변 UI를 적용했습니다.
답변 내용에 맞는 바로가기 버튼 추천도 함께 정리했습니다.
프론트 빌드 결과물도 최신 상태로 반영했습니다.
