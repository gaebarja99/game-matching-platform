...
회원 관리, 신고 관리, 정산 관리, 매출 관리, 커뮤니티 관리, 매칭방 관리, 방송 관리 추가
관리자 전용 /admin/* 라우트 구성 및 프로필 메뉴의 관리 버튼으로 진입 연결
방송 강제 종료, 경고 전송, 최근 방송 숨김/복구, 회원 상태 변경, 기간 정지, 이벤트 팡 지급 지원
준비중(CREATED) 방송도 관리자 화면에서 강제 종료 가능하도록 프론트 조건 수정
결제 및 수익화 개선

구독 결제 PortOne 연동 보강
광고 제거 결제 콜백 처리 보강
매출 관리에 구독권 매출, 광고제거 매출, 플랫폼 매출 반영
광고제거 가격 8,900원 기준 반영
관리자 매출 화면에서 스트리머 정산 추정 대신 완료된 정산 기준으로 표시하도록 변경
완료된 정산 금액은 PangWithdrawal.netPang 합계 기준으로 계산
플랫폼 매출은 구독권 매출 + 광고제거 매출 + 완료된 정산 수수료 기준으로 계산하도록 수정
방송/후원 기능 개선

후원 애니메이션 OBS 전용 URL 복구
라이브/최근 방송 카드의 LIVE 뱃지 오표시 수정
방송 시작 알림 클릭 시 해당 방송 이동 연결
팡 선물 알림 클릭 시 /profile/pang 이동 연결
후원 시 보내는 사람 팡 차감뿐 아니라 받는 사람 pangBalance에도 즉시 반영되도록 수정
운영자 지급 팡은 내 팡 > 충전 내역에서 결제 금액이 0원으로 보이도록 수정
라이브 방송 / 채팅 기능 개선

라이브 채팅에 귓속말, 쪽지 보내기, 영상 후원 표시 기능 추가
후원 카드에 영상 링크/임베드 렌더링 추가
주간 후원 랭킹 UI 유지 및 후원자 액션 연결
채팅창에 사용자 로컬 전용 지우개 버튼 추가
같은 방송 ID 재사용 시 이전 방송 채팅/후원이 섞이지 않도록 현재 방송 시작 시각 이후 메시지만 표시하도록 수정
채팅 및 운영 안전장치

라이브 채팅창, 매칭 채팅방 욕설 필터링 추가
욕설 메시지를 차단하지 않고 ** 마스킹 후 전송하도록 변경
욕설 누적 10회마다 채팅 금지 시간이 1분씩 증가하도록 서버 제재 로직 추가
라이브 채팅, DM, 그룹 채팅, 매칭 채팅에 공통 적용
커뮤니티 기능 개선

이미지 첨부 글쓰기 지원
블라인드/삭제 글 상태 처리 및 관리자 복구 동작 개선
조회수 중복 증가 완화
반응 체계를 좋아요/추천/비추천에서 저장/버프/너프로 변경
인기글 정렬 기준을 버프 순으로 변경
프로필 메뉴에서 내가 작성한 글, 저장한 글 조회 연결
친구 / 메시지 UX 개선

우측 친구 사이드바 온라인 / 오프라인 그룹 표시 복구
친구 목록 상태 표시와 핑 기반 온라인 유지 로직 보강
스트림/매칭/프로필 UI 개선

스트림 카테고리 허브 추가 및 치지직 스타일 참고 레이아웃 적용
에이펙스 포지션 이미지를 실제 아이콘으로 연결
우측 친구 사이드바 잘림 현상 수정
커뮤니티/회원관리/정산관리/매칭방관리 표 정렬 보정
프로필 좌측 메뉴 구조 정리 및 커뮤니티 동선 개선
승부 예측 페이지를 카드형 대시보드로 재구성
당첨 보관함 페이지를 요약/성과 중심 레이아웃으로 리디자인
프로필 화면의 팡, 마일리지, 활동 기간 카드 구성 정리
프로필의 보유중인 팡 카드가 실제 잔액 기준으로 맞게 보이도록 정렬
참고

일부 프론트 전체 빌드는 기존 TypeScript 오류들 때문에 여전히 실패 가능
이번 작업들은 주로 관리자, 라이브 채팅, 후원, 프로필/매출 화면 중심으로 반영됨
...
- **전적 / 이스포츠**: 각각 이미지 페이지 (`/records`, `/esports`)
- **단체 채팅**: 채팅방 목록, 초대, 실시간 메시지
- **라이브 방송**: RTMP 수신 → HLS 재생, 시청·채팅·후원
- **스튜디오**: 방송 설정, 알림, 채팅 설정, 수익·구독자 등
- **친구·DM·알림**: 친구 요청, 1:1 메시지, 알림
